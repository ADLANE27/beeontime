...
serve(async (req) => {
  ...
  try {
    ...
    // Parse request body
    const { email, password, userId, preferredId, firstName, lastName, createIfNotExists, checkOnly } = await req.json();
    ...
    throw new Error('Insufficient information provided to create or update user');
